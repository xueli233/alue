...
export default {
  ...
  methods: {
    ...
    delCartConfirm (productId){
      ...
      this.productId = productId // 产品id传进来
    },
    delCart(){
    ...
    toggleCheckAll () { // 设置全选
      let flag = !this.checkAllFlag // 如果选中,则不选
      this.cartList.forEach((item)=>{
        item.checked = flag ? 1 : 0; // 把布尔换成1,0
      })

      axios.post('users/editCheckAll',{
        checkAll: this.checkAllFlag // 把当前全选状态传过去 
      }).then((response)=>{ // 返回
        let res =response.data
        if(res.status == '0'){
          console.log(res.result)// 打印结果
        }
      })
    },
    editCart(flag, item){
